        ...
        setGridExtarnals: function (gridApi) {
            ...
            for (var i = 0; i < col.length; i++) {
                ...
                function generateCellTemplate(col, columnindex) {
                    ...
                    switch (col.type) {
                        case 'link':
                            return '<div '+attributes_for_cell+' class="' + classes_for_row + '" title="{{ COL_FIELD }}">' + prefix_img + '<a ' + (col.target ? (' target="' + col.target + '" ') : '') + ' href="{{' + 'grid.appScope.' + col.href + '}}"><i ng-if="' + col.link + '" class="fa fa-external-link" style="font-size: 12px"></i> {{COL_FIELD}}</a></div>';
                        case 'img':
                            return '<div '+attributes_for_cell+' class="' + classes_for_row + '" style="text-align:center;">' + prefix_img + '<img ng-src="{{ COL_FIELD }}" alt="image" style="background-position: center; height: 30px;text-align: center; background-repeat: no-repeat;background-size: contain;"></div>';
                        case 'show_modal':
                            return '<div '+attributes_for_cell+' class="' + classes_for_row + '" title="{{ COL_FIELD }}">' + prefix_img + '<a ng-click="' + col.modal + '" ng-bind="COL_FIELD"></a></div>';
                        case 'actions':
                        ...
                        case 'editable':
                            if (col.multiple === true && col.rule) {
                                return '<div '+attributes_for_cell+' class="' + classes_for_row + '" ng-if="grid.appScope.' + col.rule + '=== false" title="{{ COL_FIELD }}">' + prefix_img + '{{ COL_FIELD }}</div><div ng-if="grid.appScope.' + col.rule + '"><div ng-click="' + col.modal + '" title="{{ COL_FIELD }}" id=\'grid_{{row.entity.id}}\'>{{ COL_FIELD }}</div></div>';
                            }
                            if (col.subtype && col.subtype === 'tinymce') {
                                return '<div '+attributes_for_cell+' class="' + classes_for_row + '" ng-click="' + col.modal + '" title="{{ COL_FIELD }}" id=\'grid_{{row.entity.id}}\'>' + prefix_img + '{{ COL_FIELD }}</div>';
                            }
                        //TODO: SS by OZ: what is returned when neither of two above contitions is true?
                        default:
                            return '<div '+attributes_for_cell+' class="' + classes_for_row + '" title="{{ COL_FIELD }}">' + prefix_img + '{{ COL_FIELD }}</div>';
