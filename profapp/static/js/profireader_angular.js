        ...
        setGridExtarnals: function (gridApi) {
            ...
            for (var i = 0; i < col.length; i++) {
                ...
                if (col[i].type === 'link') {
                    ...
                    scope.gridOptions1.columnDefs[i].cellTemplate = '<div class="' + classes_for_row + '"><img ' +
                        'class="pr-grid-cell-type-icons-img pr-grid-cell-type-icons-img-icon-{{ icon_name }}" ng-repeat="(icon_name, icon_enabled) in COL_FIELD" ng-click="grid.appScope.' + col[i]['onclick'] + '(row.entity.id, \'{{ icon_name }}\', row.entity, \'' + col[i]['name'] + '\')" title="{{ grid.appScope._(\'grid icon \' + icon_name) }}">{{ grid.appScope._(\'grid icon \' + icon_name) }}</img></div>'
