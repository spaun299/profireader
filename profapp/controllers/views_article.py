...
def material_details_load(json, material_id):
    ...
    portals = [p.get_client_side_dict(fields='id, name, divisions.id|name|portal_division_type_id, own_company.name') for p in
               Company.get(article.company_id).get_portals_where_company_is_member()]

    for p in portals:
        for d in p['divisions']:
            d['can_be_published'] = (d['portal_division_type_id'] == 'events' or d['portal_division_type_id'] == 'news')
        p['divisions'] = PRBase.get_ordered_dict(p['divisions'])
        p['publication'] = None
        p['actions'] = ['publish', 'unpublish']
        ...
        if publication:
            p['publication'] = publication.get_client_side_dict('title,status,visibility,portal_division_id,publishing_tm')
